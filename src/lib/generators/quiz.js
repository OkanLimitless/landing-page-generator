...
export const generateQuizPage = (data) => {
  try {
    ...
    const selectedLayout = landingLayouts[Math.floor(Math.random() * landingLayouts.length)];

    // Generate both pages
    const landingPage = generateLandingPage(mergedData, styles, ids, selectedLayout, gtagAccount, gtagLabel);
    const quizPage = generateQuizPageContent(mergedData, styles, ids, gtagAccount, gtagLabel);
    ...
    console.error('Error generating quiz pages:', error);
    throw error;
  }
};

function generateLandingPage(data, styles, ids, layout, gtagAccount, gtagLabel) {
  ...
  return `<!DOCTYPE html>
    ...
    <body>
      ${layout.template(data, styles, ids)}
        ...
        document.addEventListener('DOMContentLoaded', function() {
          ...
          ctaButtons.forEach(button => {
            button.onclick = function(e) {
              if (!this.href.includes('quiz.html')) {
                e.preventDefault();
...
function generateQuizPageContent(data, styles, ids, gtagAccount, gtagLabel) {
  try {
    ...
    const productName = data.productName || 'Alpha Bites';
